...
import static io.Loader.loadStandardShader;
...
public final class OGLUtil {
	...
	public static boolean validateShader(String shaderCode, ProjectType projectType) {
	...
	public static long createWindow(int width, int height, boolean isFullscreen, SimulationScript context) {
		glfwInit();

		long window;
		String title = "Simulation window";
		if (!isFullscreen) {
		...
		if (context != null) {
			...
			Resource windowSize = new Resource(GLSLType.INT_VECTOR_2, "windowSize", size);
			context.addResource(windowSize);
			float aspect = (float) windowWidht[0] / windowHeight[0];
			Resource aspectResource = new Resource(GLSLType.FLOAT, "aspect", aspect);
	...
	public static void draw(long window, SimulationScript context) {
	...
	public static boolean compute(int x, int y, int z, SimulationScript context) {
	...
	public static Image createTexture(int width, int height, int format) {
		if (width <= 0 || height <= 0 || format <= 0) {
			return null;
		}

		glEnable(GL_TEXTURE_2D);
		Image image = new Image(glGenTextures(), width, height, format);
	...
	public static boolean deleteTexture(Image image) {
	...
	public static void passVariables(SimulationScript script) {
		if (script != null) {
...
//								System.out.println("[OPENGL] Variable " + name + " passed");
	...
	public static boolean passVariable(Resource resource, int location) {
		if (resource != null && resource.type != null) {
			...
			case GLSL_TYPE: {
				if (resource.glslType != null) {
					GLSLType type = resource.glslType;
					switch (type) {
					case BOOL: {
